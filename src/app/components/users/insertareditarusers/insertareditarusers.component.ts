...
export class InsertareditarusersComponent implements OnInit {
  ...
  roles: Role[] = [
  { idRole: 1, nombreRole: 'ADMIN', descripcionRole: 'ADMIN' },
  { idRole: 2, nombreRole: 'CLIENTE', descripcionRole: 'CLIENTE' }
  ];
  ...
  aceptar() {
    ...
    if (this.form.valid) {
      ...
      this.users.role = this.form.value.role;
      if (this.edicion) {
        //actualizar
    ...
    if (this.edicion) {
      this.uS.listId(this.id).subscribe((data) => {
        this.form = new FormGroup({
          ...
          role: new FormControl(data.role.idRole),
        });
      });
    }
  }
